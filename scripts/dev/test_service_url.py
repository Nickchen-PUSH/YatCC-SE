...
async def ensure_namespace_exists(cluster, namespace: str):
    """确保命名空间存在"""
    try:
        await asyncio.to_thread(cluster.core_v1.read_namespace, name=namespace)
...
async def create_namespace(cluster, namespace: str):
    ...
    try:
        namespace_spec = {
            ...
            "metadata": {
                "name": namespace,
                "labels": {"name": namespace, "managed-by": "yatcc-se"},
            },
        }

        await asyncio.to_thread(cluster.core_v1.create_namespace, body=namespace_spec)
...
async def main():
    ...
    parser.add_argument(
        "--user-id", type=int, default=8001, help="用户 ID (默认: 8001)"
    )
    parser.add_argument(
        "--workspace", type=str, default="test-workspace", help="工作空间名称"
    )
    parser.add_argument("--memory", type=str, default="512Mi", help="内存限制")
    parser.add_argument("--cpu", type=str, default="500m", help="CPU 限制")
    parser.add_argument(
        "--password", type=str, default="test123", help="code-server 密码"
    )
    ...
    try:
        ...
        # 4. 确保命名空间存在（仅真实集群）
        if not args.mock:
            namespace_ok = await ensure_namespace_exists(
                cluster, config.Kubernetes.NAMESPACE
            )
            if not namespace_ok:
        ...
        try:
            ...
            print(f"❌ 获取服务URL失败: {e}")
            if not args.mock:
                await debug_cluster_state(
                    cluster, job_info.id, config.Kubernetes.NAMESPACE
                )
            return
        ...
        if not args.mock:
            print("\n📖 管理命令:")
            print(
                f"🧹 清理作业: kubectl delete deployment {job_info.id} -n {config.Kubernetes.NAMESPACE}"
            )
            print(
                f"🧹 清理服务: kubectl delete service {job_info.id}-svc -n {config.Kubernetes.NAMESPACE}"
            )
            print(
                f"📋 查看Pod: kubectl get pods -l app={job_info.id} -n {config.Kubernetes.NAMESPACE}"
            )
            print(
                f"📜 查看日志: kubectl logs -l app={job_info.id} -n {config.Kubernetes.NAMESPACE}"
            )
            print(
                f"🔍 查看事件: kubectl get events -n {config.Kubernetes.NAMESPACE} --sort-by='.lastTimestamp'"
            )
...
async def debug_cluster_state(cluster, job_id: str, namespace: str):
    ...
    try:
        ...
        try:
            ns = await asyncio.to_thread(cluster.core_v1.read_namespace, name=namespace)
                    ...
                    if pod.status.conditions:
                        for condition in pod.status.conditions:
                            if condition.status == "False":
                                print(
                                    f"  - 条件问题: {condition.type} - {condition.message}"
                                )
            else:
                print("❌ 没有找到相关 Pod")
        except Exception as e:
            ...
            def check_http():
                try:
                    ...
                    req = urllib.request.Request(
                        url, headers={"User-Agent": "YatCC-SE-Script/1.0"}
                    )
