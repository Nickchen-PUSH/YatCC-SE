...
class KubernetesClusterTest(ClusterTestBase):
    ...
    def test_duplicate_job_creation_same_user(self):
        ...
        async def test_logic():
            ...
            # 验证：应该是同一个作业（用户独立部署）
            self.assertEqual(job_info_1.id, job_info_2.id, "Same user should reuse deployment")
    ...
    def test_get_service_url_port_forward(self):
        ...
        async def test_logic():
            ...
            if self.is_real_k8s:
                # 等待作业就绪
                await self.wait_for_job_status(job_info.id, JobInfo.Status.RUNNING, timeout=30)
