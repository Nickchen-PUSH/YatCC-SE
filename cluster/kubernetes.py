...
from . import (
    ...
    ClusterError, JobNotFoundError, KubernetesSpec,
)
...
class KubernetesCluster(ClusterABC):
    ...
    async def allocate_resources(self, job_params):
        ...
        job_params.name = self._generate_job_name(job_params.user_id)
    ...
    def _generate_job_name(self, user_id: str) -> str:
        """生成作业名称"""
        base_name = f"codeserver-{user_id}"
        return base_name

    async def _create_deployment(self, job_name: str, job_params: JobParams):
        """创建 Deployment"""
        deployment_spec = KubernetesSpec(job_name, job_params, self.config).build_deployment()
    ...
    async def _create_service(self, job_name: str, job_params: JobParams) -> str:
        ...
        service_spec = KubernetesSpec(job_name + "-svc", job_params, self.config).build_service()
    ...
    async def delete_job(self, job_name: str) -> List[str]:
        ...
        try:
            ...
            # 使用重试机制更新 deployment
            success = await self._suspend_deployment_with_retry(deployment_name, current_replicas)
            if success:
